...
#[tauri::command]
pub async fn query_codebuddy_cn_quota_with_binding(
    app: AppHandle,
    account_id: String,
    ...
    let started_at = Instant::now();
    logger::log_info(&format!(
        "[CodeBuddy CN Quota Query] 开始查询: account_id={}",
        account_id
    ));

    let updated = codebuddy_cn_account::query_quota_with_binding(
        &account_id,
        &cookie_header,
        product_code,
        status,
    ...
    if let Err(e) = codebuddy_cn_account::run_quota_alert_if_needed() {
        logger::log_warn(&format!(
            "[QuotaAlert][CodeBuddy CN] 手动查询后预警检查失败: {}",
            e
        ));
    }
    let _ = crate::modules::tray::update_tray_menu(&app);
    ...
    logger::log_info(&format!(
        "[CodeBuddy CN Quota Query] 查询完成: account_id={}, email={}, elapsed={}ms",
        updated.id,
        updated.email,
        started_at.elapsed().as_millis()
    ));
...
#[tauri::command]
pub fn clear_codebuddy_cn_quota_binding(
    app: AppHandle,
    account_id: String,
) -> Result<CodebuddyAccount, String> {
    logger::log_info(&format!(
        "[CodeBuddy CN Quota Query] 清除绑定: account_id={}",
        account_id
    ));
    let updated = codebuddy_cn_account::clear_quota_binding(&account_id)?;
    let _ = crate::modules::tray::update_tray_menu(&app);
    logger::log_info(&format!(
        "[CodeBuddy CN Quota Query] 清除绑定完成: account_id={}, email={}",
