...
fn parse_codebuddy_cn_quota_curl_request(
    raw: &str,
...
pub async fn query_quota_with_binding(
    ...
    let mut normalized_cookie_header: String;
    ...
    let should_register_prewarm = account
        .uid
        .as_deref()
        ...
        && account
            ...
            .map(|v| v.trim().is_empty())
            .unwrap_or(true)
        && extract_profile_uin(account.profile_raw.as_ref()).is_none();

    let user_resource = if is_manual_curl_replay {
        let parsed_curl = match parse_codebuddy_cn_quota_curl_request(cookie_header) {
            Ok(parsed) => parsed,
            Err(err) => {
        ...
        match codebuddy_cn_oauth::fetch_user_resource_with_raw_request(
        ...
        match codebuddy_cn_oauth::fetch_user_resource_with_cookie_retry_register(
        ...
        .await
        {
            Ok((value, refreshed_cookie)) => {
                if let Some(new_cookie_header) = refreshed_cookie {
                    normalized_cookie_header = new_cookie_header;
                }
                value
            }
            Err(err) => {
    ...
    let dosage = match codebuddy_cn_oauth::fetch_dosage_notify(
    ...
    let payment = match codebuddy_cn_oauth::fetch_payment_type(
        &account.access_token,
        account.uid.as_deref(),
        account.enterprise_id.as_deref(),
        account.domain.as_deref(),
